...
const SharedItinerary: React.FC<SharedItineraryProps> = ({
  ...
  const currentToken = shareToken || urlToken
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                  <div className="flex items-center gap-2">
                    <FaShare className="text-xl sm:text-2xl text-blue-600 flex-shrink-0" />
                    <h1 className="text-lg sm:text-2xl font-bold text-gray-900 truncate">{itinerary.title}</h1>
                  </div>
                  <span className="self-start px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                    Shared
                  </span>
                </div>
                <p className="text-sm sm:text-base text-gray-600 line-clamp-2">{itinerary.description}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
              <button
                onClick={handleLike}
                className={`flex items-center px-2 sm:px-3 py-2 rounded-lg transition-all text-sm ${
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-sm text-gray-600">
            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
              <div className="flex items-center">
                ...
                <span className="whitespace-nowrap">{collaborators.length} collaborators</span>
              </div>
              <div className="flex items-center">
              ...
              <div className="flex items-center">
                <span className={`px-2 py-1 rounded text-xs whitespace-nowrap ${getRoleColor(userRole)}`}>
                  Your role: {userRole}
                </span>
              </div>
            </div>
                ...
                <div className="flex overflow-x-auto scrollbar-thin">
                  {itinerary.days.map((day, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium transition-all min-w-max ${
                        selectedDay === index
                          ? 'bg-white text-blue-600 border-b-2 border-blue-600'
                          : 'text-gray-600 hover:text-gray-900'
                      }`}
                    >
                      <div className="text-sm sm:text-base">Day {index + 1}</div>
          ...
          <div className="lg:col-span-1 order-1 lg:order-2 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base sm:text-lg font-semibold">Collaborators</h3>
                <Button
